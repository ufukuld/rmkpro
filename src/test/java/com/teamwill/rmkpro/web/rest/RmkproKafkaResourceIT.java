...
import com.teamwill.rmkpro.config.KafkaProperties;
...
class RmkproKafkaResourceIT {
    ...
    void setup() {
        ...
        RmkproKafkaResource kafkaResource = new RmkproKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/rmkpro-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/rmkpro-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
